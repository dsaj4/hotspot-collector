...
material-hub-workspace/... -> %HOTSPOT_DATA_ROOT%/material-hub-workspace/...
trend-intake-workspace/... -> %HOTSPOT_DATA_ROOT%/trend-intake-workspace/...
...
Material-hub and content-system preparation files default to the data root:

```text
MATERIAL_WORKSPACE_ROOT=%HOTSPOT_DATA_ROOT%
```

Override `MATERIAL_WORKSPACE_ROOT` only when you intentionally mount a separate local material workspace. Do not use another source repository as the default runtime workspace.
